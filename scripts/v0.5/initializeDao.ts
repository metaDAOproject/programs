...
import { AutocratClient, getDaoAddr, DEVNET_USDC, MAINNET_USDC, SQUADS_PROGRAM_CONFIG_TREASURY } from "@metadaoproject/futarchy/v0.5";
...
const EXISTING_TOKEN = new PublicKey("METADDFL6wWMWEoKTFJwcThTbUmtarRJZjRpzUvkxhr");
...
export const initializeDao = async () => {
  ...
  let _SQUADS_PROGRAM_CONFIG_TREASURY: PublicKey = SQUADS_PROGRAM_CONFIG_TREASURY;
  ...
  const response = await fetch(`https://lite-api.jup.ag/price/v3?ids=${EXISTING_TOKEN.toBase58()}`);
