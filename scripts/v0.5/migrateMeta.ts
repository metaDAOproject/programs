...
import { AutocratClient, getDaoAddr, getMetadataAddr, SQUADS_PROGRAM_CONFIG_TREASURY, MAINNET_USDC, DEVNET_USDC, USDC_DECIMALS } from "@metadaoproject/futarchy/v0.5";
import { BN } from "bn.js";
import { assert } from "chai";
import * as multisig from "@sqds/multisig";
import { createUmi } from "@metaplex-foundation/umi-bundle-defaults";
import { createV1, mplTokenMetadata, TokenStandard, updateMetadataAccountV2 } from "@metaplex-foundation/mpl-token-metadata";
import { percentAmount, signerIdentity, publicKey as UmiPublicKey } from "@metaplex-foundation/umi";
...
// TOKEN DETAILS
let TOKEN_MINT: PublicKey | null = new PublicKey("METAwkXcqyXKy1AtsSgJ8JiUHwGCafnZL38n3vYmeta");
const TOKEN_SEED: string = 'j0MH5YqwvFHs2h9T';
const NEW_TOKEN_DECIMALS: number = 6;
const CLASSIC_TOKEN_MINT: PublicKey = new PublicKey("METADDFL6wWMWEoKTFJwcThTbUmtarRJZjRpzUvkxhr");
...
const mintNewToken = async () => {
  ...
  if (provider.connection.rpcEndpoint.includes("devnet")) {
    ...
    console.log("Using mainnet params.. MAKE SURE YOU WANT TO DO THIS... Sleeping for 10 seconds")
    await new Promise(resolve => setTimeout(resolve, 10000));
  }

  const tokenDecimalDiff = CLASSIC_TOKEN_DECIMALS - NEW_TOKEN_DECIMALS;

  const classicTokenSupply = await provider.connection.getTokenSupply(CLASSIC_TOKEN_MINT);
  const classicTokenSupplyAmount = classicTokenSupply.value.amount;

  const classicTokenSupplyAmountBN = new BN(classicTokenSupplyAmount.toString());
  ...
  try {
    if (TOKEN_MINT) {
      const newTokenSupply = await provider.connection.getTokenSupply(TOKEN_MINT);
      newTokenSupplyAmount = new BN(newTokenSupply.value.amount.toString());
      console.log('New token already exsits. You may need to mint more or change something.');
      console.log('New token supply (human readable):', newTokenSupplyAmount.div(new BN(10).pow(new BN(NEW_TOKEN_DECIMALS))).toString());

      assert(newTokenSupplyAmount.eq(classicTokenSupplyAmountBN), "New token supply is not equal to 1:1000 ratio of classic token supply");
      return TOKEN_MINT;
    }
  } catch (error) {
    console.log(error);
  ...
  if(!newTokenSupplyAmount.gt(new BN(0))) {
    ...
    const initializeMintIx = createInitializeMint2Instruction(TOKEN, NEW_TOKEN_DECIMALS, payer.publicKey, null);
    const associatedTokenAccount = getAssociatedTokenAddressSync(TOKEN, payer.publicKey, true);
    ...
    const classicTokenSupplyHuman = classicTokenSupplyAmountBN.div(new BN(10).pow(new BN(CLASSIC_TOKEN_DECIMALS)))
    const newTokenSupplyHuman = classicTokenSupplyAmountBN.div(new BN(10).pow(new BN(NEW_TOKEN_DECIMALS)));
    ...
    const txHash = await provider.connection.sendRawTransaction(versionedTx.serialize());
    ...
    console.log('New token supply (human readable):', new BN(newTokenSupply.value.amount.toString()).div(new BN(10).pow(new BN(NEW_TOKEN_DECIMALS))).toString());
  }
  return TOKEN
}
...
const initializeNewDaoAndToken = async () => {
  ...
  if (provider.connection.rpcEndpoint.includes("devnet")) {
    console.log("Using devnet params")
    USDC = DEVNET_USDC;
    squadsProgramConfigTreasury = new PublicKey("HM5y4mz3Bt9JY9mr1hkyhnvqxSH4H2u2451j7Hc2dtvK");  // NOTE: THIS IS FOR DEVNET ONLY
  } else {
  ...
  const ixs = [];
  const initializeDaoIx = await autocrat.initializeDaoIx({
    baseMint: TOKEN,
    params: {
      ...
      minBaseFutarchicLiquidity: new BN(MIN_BASE_FUTARCHIC_LIQUIDITY).mul(new BN(10).pow(new BN(NEW_TOKEN_DECIMALS))),
      minQuoteFutarchicLiquidity: new BN(MIN_QUOTE_FUTARCHIC_LIQUIDITY).mul(new BN(10).pow(new BN(USDC_DECIMALS))),
      passThresholdBps: PASS_THRESHOLD_BPS,
      slotsPerProposal: THREE_DAYS_IN_SLOTS,
      initialSpendingLimit: {
        amountPerMonth: new BN(SPENDING_LIMIT).mul(new BN(10).pow(new BN(USDC_DECIMALS))),
  ...
  const updateMetadataInstructions = umiUpdateInstructions.map(umiInstruction => 
    toWeb3JsInstruction(umiInstruction)
  );
  ...
  try {
    ...
    const transferMintAuthorityTx = createSetAuthorityInstruction(TOKEN, payer.publicKey, AuthorityType.MintTokens, squadsMultisigVault);
