...
async function main() {
  ...
  const fundingRecordsDir = "funding_records";
  if (!fs.existsSync(launchesDir)) {
    fs.mkdirSync(launchesDir);
  }
  if (!fs.existsSync(fundingRecordsDir)) {
    fs.mkdirSync(fundingRecordsDir);
  }
  ...
  console.log(
    `FundingRecord discriminator (base58): ${bs58.encode(fundingRecordDiscriminator)}`,
  );
  ...
  for (const { pubkey } of fundingRecordAccounts) {
    await dumpAccount(pubkey, fundingRecordsDir, "FundingRecord");
  }
}
