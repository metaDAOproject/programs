import { AmmClient, AUTOCRAT_PROGRAM_ID, AutocratClient, ConditionalVaultClient, getProposalAddr } from "@metadaoproject/futarchy/v0.4";
...
async function main() {
  ...
  // TODO: You might want to override this or adjust the nonce etc
  const [metaDaoProposal] = getProposalAddr(AUTOCRAT_PROGRAM_ID, payer.publicKey, new BN(0));
  ...
  const {
    ...
    passLp, failLp
  } = autocratClient.getProposalPdas(metaDaoProposal, dao.tokenMint, dao.usdcMint, DAO_KEY);
  ...
  const mergeTokens = await vaultClient.mergeTokensIx(question, baseVault, dao.tokenMint, baseTokensToMergeAmount, 2, payer.publicKey).rpc();
  const mergeTokensQuote = await vaultClient.mergeTokensIx(question, quoteVault, dao.usdcMint, quoteTokensToMergeAmount, 2, payer.publicKey).rpc();

  // If the proposal is complete in some way then you can redeem the tokens
