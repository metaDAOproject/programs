...
const outcomeQuestionText = await input({
  message:
    "Enter the outcome question text (example: Will Jito approve Switchboard's 300k JTO NCN Grant?/No/Yes):\n",
});
const metricQuestionText = await input({
  message:
    "Enter the metric question text (example: Will Switchboard's NCN generate more than $1M by 12/01/25?/No/Yes):\n",
});

const liquidityAmount = await input({
  message:
    "Enter the amount of USDC to provide as liquidity (example: 1000, must be at least 100):\n",
});
