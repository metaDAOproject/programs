...
impl FuzzTest {
    pub fn verify_initialize_performance_package_invariants(
        ...
        assert_eq!(pp.mint, token_mint, "Stored mint must match the mint account");
        assert_eq!(
            pp.mintGovernor, mint_governor,
            "Stored mintGovernor must match the mint_governor account"
        );
        assert_eq!(
            pp.mintAuthority, mint_authority,
        ...
        assert_eq!(
            pp.oracleReader, args.oracleReader,
            "Stored oracleReader must match args.oracleReader"
        );
        assert_eq!(
            pp.rewardFunction, args.rewardFunction,
        ...
        match (&pp.oracleReader, &args.oracleReader) {
            ...
            (
                ...
                assert_eq!(*amm, *expected_amm, "Stored FutarchyTwap amm must match args");
                assert_eq!(
                    *minDuration, *expected_min_duration,
