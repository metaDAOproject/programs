...
export default function suite() {
    ...
    it("does conditional swaps", async function () {
        const { proposal, question, quoteVault, squadsProposal } = await this.initializeAndLaunchProposal({
            dao,
            instructions: [{
        ...
        const { passBaseMint, passQuoteMint, failBaseMint, failQuoteMint } = this.futarchy.getProposalPdas(proposal, META, USDC, dao);

        // Split some tokens to have conditional tokens to trade
        await this.conditionalVault.splitTokensIx(question, quoteVault, USDC, new BN(50 * 10 ** 6), 2).rpc();
        ...
        const postPassQuoteBalance = await this.getTokenBalance(passQuoteMint, this.payer.publicKey);
        const postPassBaseBalance = await this.getTokenBalance(passBaseMint, this.payer.publicKey);

        assert.ok(postAmmState.totalLiquidity.eq(preAmmState.totalLiquidity));

        // First check all the fees
        assert.equal(postAmmState.state.futarchy.spot.quoteProtocolFeeBalance.toString(), "0");
        assert.equal(postAmmState.state.futarchy.spot.baseProtocolFeeBalance.toString(), "0");
        assert.equal(postAmmState.state.futarchy.pass.quoteProtocolFeeBalance.toString(), "25000"); // 2.5 cent fee on $100 swap
        assert.equal(postAmmState.state.futarchy.pass.baseProtocolFeeBalance.toString(), "0");
        assert.equal(postAmmState.state.futarchy.fail.quoteProtocolFeeBalance.toString(), "0");
        assert.equal(postAmmState.state.futarchy.fail.baseProtocolFeeBalance.toString(), "0");
        ...
        const postFailQuoteBalance = await this.getTokenBalance(failQuoteMint, this.payer.publicKey);
        const postFailBaseBalance = await this.getTokenBalance(failBaseMint, this.payer.publicKey);
    ...
    it("fails when user has insufficient balance", async function () {
        ...
        // Split some tokens to have conditional tokens to trade
        await this.conditionalVault.splitTokensIx(question, quoteVault, USDC, new BN(5 * 10 ** 6), 2).rpc();

        const callbacks = expectError("InsufficientBalance", "conditional swap should fail with insufficient balance");
    ...
    it("fails when proposal is not in pending state", async function () {
        ...
        const callbacks = expectError("ProposalNotActive", "conditional swap should fail when proposal is not active");
    ...
    it("respects minimum output amount", async function () {
        ...
        const callbacks = expectError("SwapSlippageExceeded", "conditional swap should fail when slippage is too high");
