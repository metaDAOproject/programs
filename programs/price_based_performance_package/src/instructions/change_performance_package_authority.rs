...
use crate::{CommonFields, PerformancePackage, PerformancePackageAuthorityChanged, PriceBasedPerformancePackageError};
...
impl<'info> ChangePerformancePackageAuthority<'info> {
    pub fn handle(ctx: Context<Self>, params: ChangePerformancePackageAuthorityParams) -> Result<()> {
        let Self {
            performance_package,
            current_authority: _,
        } = ctx.accounts;
